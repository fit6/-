...
def train(model, iterator, optimizer, criterion, file_writer,epoch):
    model.train()
    for i, batch in enumerate(tqdm(iterator)):
        ...
        loss.backward()
        optimizer.step()
        file_writer.add_scalar('loss/train', loss, epoch)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, hp.n_epochs + 1):
        ...
        train(model, train_iter, optimizer, criterion, file_writer,epoch)

        fname = os.path.join(hp.logdir, str(epoch))
        print("=========dev at epoch={}=========".format(epoch))
        trigger_f1, argument_f1 = eval(model, dev_iter, fname + '_dev',file_writer,epoch)
