...
class Metadata(object):
    def __init__(self, metadata_path='./data/metadata.json'):
        ...
        self.DuEE = DatasetFactContainer(meta['DuEE'])
...
class DatasetFactContainer(object):
    def __init__(self, trigger_arg_dic, arg_entity_dic=None):
        ...
        self.args_to_mappings, self.mappings_to_args = {}, {}
    ...
    def init_setup(self):
        ...
        self.args_to_mappings, self.mappings_to_args = self.set_to_mappings(self.arg_set)
    ...
    @staticmethod
    def set_to_mappings(input_set):
        items_to_ids = dict((v, i) for v, i in zip(sorted(list(input_set)), range(len(input_set))))
        items_to_ids['[SEP]'] = len(items_to_ids)
        items_to_ids['[PAD]'] = len(items_to_ids)
        ids_to_items = dict((v, k) for k, v in items_to_ids.items())
        return items_to_ids, ids_to_items
