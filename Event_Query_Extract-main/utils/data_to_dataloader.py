...
def _unpack_arg_DuEE(data, tokenizer):
    """
    read the data with pos tags
    :param data:
    :param tokenizer:
    ...
    # if n > 4:
        # mid = (n-2) // 2
    triggers = [x.split(' ') for x in data[1:n-1:2]]
    arguments = [x.split(' ') for x in data[2:n-1:2]]
    trigger_count[0] += len(triggers)
    # else:
    #     triggers = [x.split(' ') for x in data[1]]
    #     arguments = [x.split(' ') for x in data[2]]
    #     trigger_count[0] += len(triggers)

    return sentence, bert_words, triggers, arguments
...
def read_data_from(file, tokenizer, data_split, DuEE=True, ace=False, with_vt=False):
    """
    ...
    if data_split == 'dev':
        ...
        output2 = []
        output3 = []
        file1 = './data/DuEE1.0/after-process/train.doc2.txt'
        ...
        output2_choices = []
        output_pad_choices = []
        output1_choices = sample(output1,208)
        output2_choices = sample(output2,150)
        output_pad_choices = sample(output3,392)
        output_ =  output1_choices  + output_pad_choices
    else:
        output_ = output_all 
    ...
    elif DuEE and not with_vt:
        output = list(map(partial(_unpack_arg_DuEE, tokenizer=tokenizer), output_))
        # r = r+1
    # print(r)
    return output
