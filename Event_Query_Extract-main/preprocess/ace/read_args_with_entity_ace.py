...
def read_arguments_from_ace(ace_path, SEN_SEP='SENT_SEP'):
    ...
    for i in range(len(data)):
        # read triggers
        _sentence, _token_offsets, _event_type, _trigger_idx, _arguments = data[i]
        print(data[i])
        time.sleep(10)
        ...
        output.append(sum([[list(_sentence)], this_triggers, this_arguments, []], []))
    return output
...
def gen_arguments_bio(sent, offsets, arguments_info):
    ...
    args = [x for x in re.split('[ :#@#]', arguments_info) if x]
    print(args)
    _n = len(args)
    _arguments, _idx_begin, _idx_end = args[0:_n:4], args[2:_n:4], args[3:_n:4]
...
def read_ace_dir_to_bio(ace_files, output_path):
    ...
    data = read_arguments_from_ace(files)
    for i in range(len(data)):
        data[i].append(files.split('/')[-1])
    output.extend(data)
