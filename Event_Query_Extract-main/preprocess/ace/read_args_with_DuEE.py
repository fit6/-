...
spacy_tagger = spacy.load("zh_core_web_sm")
...
def read_arguments_from_ace(ace_path, SEN_SEP='SENT_SEP'):
    ...
    #     pass
    role_files, trigger_files = ace_path
    df_role = pd.read_csv(role_files, sep='\t')
    df_trigger = pd.read_csv(trigger_files, sep='\t')
    df_role = df_role.fillna(SEN_SEP)
    df_trigger = df_trigger.fillna(SEN_SEP)
    # data = extract_data(df_ace)

    output = []
    # trigger_c = 0
    trigger_text = df_trigger['text_a'].values
    trigger_label = df_trigger['label'].values
    role_text = df_role['text_a'].values
    role_label = df_role['label'].values
    y = trigger_text[0][1]
    output = []
    _sentence = []
    for x in trigger_text:
        _sentence.append(''.join(x.split(y)))
    this_triggers = read_triggers(df_trigger)
    print('start read arguments')

    # read arguments
    this_arguments = read_arguments(df_role)

    # read entities
    # ner = processed_entities(_token_offsets, _ner_offset, _ner_type)
    for index, sentence in enumerate(_sentence):
        output.append(sum([[list(sentence)], [this_triggers[index]], [this_arguments[index]], []], []))
...
def read_arguments(files):
    arguments_bio = gen_arguments_bio(files)
    return arguments_bio


def gen_arguments_bio(files):
    arguments_bio = read_args(files)
...
def read_args(files):
    ...
    :return: [[BIO for trigger1], [BIO for trigger2], ...]
    """
    role_text = files['text_a'].values
    role_label = files['label'].values
    y = role_text[0][1]
    arg_bio = []
    for x in role_label:
        arg_bio.append(x.split(y))
...
def read_triggers(files):
    ...
    :return: [[BIO for trigger1], [BIO for trigger2], ...]
    """
    trigger_text = files['text_a'].values
    trigger_label = files['label'].values
    y = trigger_text[0][1]
    output = []
    for x in trigger_label:
        output.append(x.split(y))
...
def read_ace_dir_to_bio(ace_files, output_path):
    ...
    data = read_arguments_from_ace(files)#(sentence，sum([[list(sentence)], this_triggers[index], this_arguments[index], []], []))
    for i in range(len(data)):
        data[i].append(files[0].split('/')[-1])
    output.extend(data)
...
def read_ace_with_entity(data_dir, output_dir):
    """
    ...
    :return: None
    """
    splits = ['train.doc.txt', 'dev.doc.txt']
    ...
    for data in splits:
        ...
        # files = open(split_path + 'duee_'+data[:-8], 'r').read().splitlines()
        files = [data_dir+'role/'+data[:-8]+'.tsv',data_dir+'trigger/'+data[:-8]+'.tsv']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', default='../../data/DuEE1.0/', type=str, required=True)
    # parser.add_argument('--split_path', default='../../data/splits/', type=str, required=True)
    parser.add_argument('--output', default='../../data/DuEE1.0/after-process/', type=str, required=True)
    args = parser.parse_args()
    assert args.ere or args.ace is True, 'set either ACE or ERE with --ace or --ere'
    if args.ace:
        read_ace_with_entity(args.path, args.output)
    else:
        pass
