...
import numpy as np
import time
from model.trigger_detection.train import PrepareData
from model.trigger_detection.gat_model import GAT
...
class TriggerDetection(nn.Module):
    def __init__(self, config):
        ...
        if self.pretrained_weights == 'distilbert-base-uncased':
            ...
            self.bert = BertModel.from_pretrained('./data/chinese_wwm_ext_pytorch', output_attentions=True)
            ...
            self.bert = BertModel.from_pretrained('./data/roberta_chinese', output_attentions=True)
        else:
            self.bert = None

        self.linear = nn.Sequential(
            nn.Dropout(p=self.config.dropout),
            nn.Linear(self.embedding_dim*4+17, 2),
        )
        self.sigmoid = nn.Sigmoid()
        self.W = nn.Parameter(torch.rand(self.embedding_dim, self.embedding_dim))
        self.W1 = nn.Parameter(torch.rand(self.embedding_dim, self.embedding_dim))

        self.event_count = config.event_count
        self.last_k_hidden = config.last_k_hidden
        self.sqrt_d = np.sqrt(self.embedding_dim)
        self.cos = nn.CosineSimilarity(dim=-1)
        self.preparedata = PrepareData('train_graph')
        self.event_featuer = self.preparedata.event_features.cuda()
        self.adj = self.preparedata.adj.cuda()
        self.gat_model = GAT(nfeat=self.event_featuer.shape[1], 
                nhid=128, 
                nclass=768, 
    ...
    def forward(self, dataset_id, sentence_batch, idxs_to_collect_sent, idxs_to_collect_event, embed_lengths, pos_tag):
        ...
        # print((sentence_batch == 102).nonzero(as_tuple=True)[1].reshape(sentence_batch.shape[0], -1))
        ...
        # print(pos_tag.shape)
        event_featuer, adj = Variable(self.event_featuer), Variable(self.adj)
        event_output = self.gat_model(event_featuer, adj)
        event_output = event_output.expand([sent_embeddings.shape[0], event_output.shape[0], event_output.shape[1]])
        logits2 = self.cos(sent_embeddings.unsqueeze(2).matmul(self.W1), event_output.unsqueeze(1))
        sent2event_att2 = logits2.matmul(event_output) * map_numerator
        # print(sent2event_att2.shape)
        ...
        _logits = self.linear(torch.cat((sent_embeddings, sent2event_att2, context_emb, pos_tag[:, :context_emb.shape[1]],sent2event_att), dim=-1))
        return _logits
